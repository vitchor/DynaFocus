...
//	if(abs(HReference.at<double>(2,0) > 0.0001) || abs(HReference.at<double>(2,1) > 0.0001)){
//		index = 2;
//		HReference = (Mat_<double>(3,3) << 1, 0, 0, 0, 1, 0, 0, 0, 1);
//	}
...
void blurImages(Mat &src1, Mat &src2, Mat &output1, Mat &output2){
...
void AntiShake::antiShake(Mat &img_1, Mat &img_2) {
    ...
    // STEP 0: DECREASE RESOLUTION
    Mat workImage1, workImage2;
    workImage1.create(0.6*img_1.rows, 0.6*img_1.cols, img_1.type());
    workImage2.create(0.6*img_2.rows, 0.6*img_2.cols, img_2.type());
...
//	// STEP 2: BLUR EVERYTHING TO NORMALIZE THE SOURCE IMAGES
	blurImages(workImage1, workImage2, workImage1, workImage2);
    cout << "=== STEP 2 complete: compensate Blurriness" << endl;

	// STEP 3: KeyPoint Detection:
	//	cv::FeatureDetector *detector = new cv::DenseFeatureDetector()
	cv::FeatureDetector *detector = new cv::FastFeatureDetector(1, true);//TODO
...
//	cv:: DescriptorExtractor *extractor = new cv::SIFT();
	...
	this->getBestMatches(MAX(25, 0.01*MIN(keypoints_1.size(), keypoints_2.size())) , good_matches, pts1, pts2, descriptors_1, descriptors_2, keypoints_1, keypoints_2);
//	Mat img_matches;
//	drawMatches(img_1, keypoints_1, img_2, keypoints_2, good_matches,
//			img_matches, Scalar::all(-1), Scalar::all(-1), vector<char>(),
//			DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
//	displayWindow(img_matches, "MATCHES");
    cout << "==== STEP 5 complete: finished matching descriptors" << endl;
