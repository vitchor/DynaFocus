...
- (void) continueToFacebookLoginButtonAction {
    ...
    [appDelegate openSessionWithTag:UPLOADING];
...
- (IBAction) changedSwitchValue: (id) sender {
    ...
    if (fbSwitch.on) {
        if (FBSession.activeSession.state == FBSessionStateCreatedTokenLoaded) {
            ...
            AppDelegate* appDelegate = [UIApplication sharedApplication].delegate;
            [appDelegate openSessionWithTag:SHARING];
        }
    }
}
...
- (void) upload
{
    ...
    NSLog(@"SENDING IMAGE REQUEST WITH:");
    NSLog(@"NAME: %@", savedFacebookName);
    NSLog(@"ID: %@", savedFacebookId);
    NSLog(@"EMAIl: %@", savedFacebookEmail);
    ...
    NSLog(@"PRINTING REQUEST: %@", request.description);
